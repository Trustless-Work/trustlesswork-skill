...
4. **[ENFORCED]** Completion ≠ approval: the `serviceProvider` marking a milestone complete moves no funds. Only the `approver` role assignment's separate approval satisfies the approval precondition. Status/evidence updates stay allowed while a dispute is open (fresh evidence can help resolve it) but are rejected once the state is terminal — single-release: escrow `released`/`resolved` (`EscrowAlreadyReleased`/`EscrowAlreadyResolved`); multi-release: that milestone `released`/`resolved` (`MilestoneAlreadyReleased`/`MilestoneAlreadyResolved`). Added to the V1 lineage on 2026-09-15 (commits `22a0b23` single / `887182a` multi); deployments predating them still accept these updates.
