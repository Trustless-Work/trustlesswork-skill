...
**[ENFORCED]** Funds live in a Soroban smart-contract escrow on Stellar — never in Trustless Work's or the platform's custody. Write endpoints return an **unsigned XDR transaction**; only a signature from the correct role's wallet makes it valid.
...
**[ENFORCED]** Authority comes exclusively from holding a role address in the escrow. The smart contract enforces this — a UI cannot grant what the contract denies. Flows that assume an actor can perform an action their role does not permit will fail on-chain.
...
5. **[ENFORCED]** Single-release: **all** milestones must be approved and no dispute active before `release-funds`; the contract then releases the entire balance minus fees in one payment.
...
7. **[ENFORCED]** Dispute resolution is terminal: the resolver's `distributions` must sum exactly to the current escrow balance (post-fees). Once resolved, a dispute cannot be reopened.
...
1. **[ENFORCED]** `platformFee` is a **fee-rate configuration** set at deploy — not an extra token amount to fund. Fund the escrow `amount` (single-release) or the sum of milestone amounts (multi-release); nothing more.
2. **[ENFORCED]** At release, the contract computes the deductions from the escrow amount — the platform fee (sent to `platformAddress`) and the Trustless Work protocol fee — and transfers the remainder to the `receiver`.
3. **[FACT]** The protocol fee is currently a fixed **0.3%** on mainnet.
4. **[ENFORCED]** The API rejects `platformFee` values exceeding 99%.
...
1. **Never work around an [ENFORCED] law.** If a requested feature violates one — a dispute button for the `receiver`, editing a funded escrow's amount, string amounts, server-side signing — flag the conflict to the user instead of implementing it. A **[CANONICAL]** deviation that remains technically valid is acceptable; note the deviation.
...
**Version**: 1.1.0 — based on **v1** of the official documentation and spot-checks against the deployed testnet API, as of 2026-08-26. Documentation v2 is under development; this file must be re-validated and amended against v2 when it is published.
